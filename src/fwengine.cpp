...
// Last-Updated: Fri Jan 21 03:20:58 2011 (+0100)
//           By: Vinay Jethava
//     Update #: 224
// URL:
// Keywords:
    ...
    for (int i = 0; i < attrSizes[pi1.first]; i++) {
      for (int j = 0; j < attrSizes[pi1.second]; j++) {
        ...
        case MLE:
          ...
          val10 = (double) 1.0/((double) cVarSet->nrStates());
...
vector<size_t> FwEngine::solveForQuery(const vector<string>* words, const vector<int>* freq, map<string, vector<Query* > * >* sqMpPtr) {
  ...
  if (!clFactorsFlag) {
    ...
    this->makeChowLiuFactors(OTHER, 0.99);
  }
...
int FwEngine::getModelName() {
  int a;
  if ((onlyWords==1) && (useWN == 0))
    a = 0; //"BASE";
  else if ((onlyWords==1) && (useWN == 1))
    a = 1; //"WN";
  else if ((onlyWords == 0) && (useWN == 0))
    a = 2; //"CL";
  else
    a = 3; //"CLWN";
  return a;
...
int FwEngine::writeRes(vector<vector<size_t> >& output, vector<Query*>& samples, string dirName, vector<double>* accRes) {
  ...
  ofstream logger("logger2.csv", ios::app);
  int model = this->getModelName();
  logger << model;
  ...
  /// vector containing distribution of hamming errors encountered
  int hammingV[numAttrs+1];
  for(int i=0; i <numAttrs+1; i++) hammingV[i] = 0;
  for (int i = 0; i < numAttrs; i++) {
    sumV[i] = 0.0;
  }
  for (int i = 0; i < output.size(); i++) {
    ...
    assert(hammingCount <= numAttrs);
    hammingV[hammingCount]++;
  }
  ...
  stringstream ss123(""); 
  // logger << "," << MAX_DEPTH  ;
  logger<<","  << unseenWords  << ","<<missingWN->size() <<"," <<missingDB->size() <<"\n";// << expNum << ","<< levelFactor<<"\n";
  //  logger << ss123.str(); 
  logger.close();
...
int FwEngine::testSolver(double p, bool useWN, bool onlyCL, bool onlyWords, int num) {
  ...
  fprintf(stderr, "Training on %d samples\n", (int) trQ->size());
  time_t tb, tf;
  tb = time(NULL);
